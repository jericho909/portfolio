...
<header class={"sticky z-[10] top-0 duration-200 px-6 flex items-center justify-between border border-solid " + (
    ...
    <h1 class="font-medium">
        <b class="font-bold poppins">Ömer Ahmet</b> Yücel
    </h1>
    <div class="sm:flex items-center gap-4 hidden">
        {#each tabs as tab, index}
            <a href="tab.link">
                <p class="duration-200 hover:text-violet-400">{tab.name}</p>
            </a>
        {/each}
    
    <a href="https://www.linkedin.com/in/omerahmetyucel/" target="_blank" class="blueShadow relative overflow-hidden px-5 py-2 group rounded-full bg-white text-slate-950">
